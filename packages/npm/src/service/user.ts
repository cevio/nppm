...
import { ORM_CONNECTION_CONTEXT, REDIS_CONNECTION_CONTEXT, RADOX_APPLICATION_CONTEXT } from '@nppm/utils';
...
export class UserService {
  ...
  get radox() {
    return RADOX_APPLICATION_CONTEXT.value;
  }
  ...
  public async login(session: string, body?: { hostname: string }) {
    ...
    if (!session) throw new HttpNotAcceptableException('请使用NPM的命令行工具登录');
    if (!this.HttpService.logins.has(configs.login_code)) {
  ...
  public async authorize(session: string) {
    ...
    if (!(await this.redis.exists(key))) {
      throw new HttpLockedException('非法操作');
    }
    const configs = await ConfigCacheAble.get();
